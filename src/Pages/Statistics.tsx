...
function Statistics(props:Props) {
    ...
    function extractTimeFromString(dateTimeString: string): string {
          ...
          const updatedIssues = await Promise.all(issues.map(async (issue) => {
            try {
              ...
              return { ...issue, averageVotes: response.data };
                ...
                console.error(`Error fetching average vote for issue ${issue.issueId}:`, error);
                return issue;
            }
        }));
      ...
      const fetchIssues = async () => {
          const response = await axios.get(props.url+"/issue/"+props.selectedProject, {
              headers: {
                  'Authorization': `Bearer ${token}`
              }
          });
  ...
  return (
    <div>
              <div>
             <h1>Statistik</h1>
             <button onClick={props.onBackToHome} className="backButton">Spela planingpoker</button>
            {issues.length > 0 ? (
